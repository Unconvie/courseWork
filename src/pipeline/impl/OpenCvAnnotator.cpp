...
// ─── ПОДПИСИ И ЦВЕТА НА *-shapeOutline ───────────────────────────────────
// OpenCV использует BGR. Меняй числа здесь, если нужны другие цвета/толщина.
...
void drawLabel(cv::Mat& canvas, const std::string& text, cv::Point origin,
  ...
  // Чёрная обводка текста — читаемость на белом фоне.
  cv::putText(canvas, text, pos, cv::FONT_HERSHEY_SIMPLEX, 0.55,
              cv::Scalar(0, 0, 0), 4, cv::LINE_AA);
  cv::putText(canvas, text, pos, cv::FONT_HERSHEY_SIMPLEX, 0.55, color, 2,
              cv::LINE_AA);
}
...
core::Image OpenCvAnnotator::annotate(
  ...
  constexpr int kThickness = 3;  // толщина контура и bbox
  ...
  for (const auto& obj : objects) {
    ...
    if (!obj.detection.contour.empty()) {
      ...
      cv::drawContours(canvas, contours, -1, color, kThickness, cv::LINE_AA);
    }

    const auto& b = obj.detection.bbox;
    cv::rectangle(canvas, cv::Rect(b.x, b.y, b.width, b.height), color,
                  kThickness, cv::LINE_AA);

    // Текст подписи: имя из ShapeKind.cpp + уверенность top-класса.
    char label[128];
