...
PipelineResult Pipeline::process(const core::Image& input) {
  ...
  // Шаг 1: картинка для отчёта *-noiseFree (на детекцию не влияет).
  result.denoised = m_noiseFilter->filter(input);

  // Шаг 2: кандидаты (контуры + bbox). Важно: исходник, не denoised —
  // иначе тонкие обводки размываются.
  ...
  for (auto& detection : detections) {
    ...
    // Шаг 3: числовые признаки контура (vertexCount, circularity, …).
    obj.features = m_featureExtractor->extract(input, detection);

    // Шаг 4: сравнение с эталонами Circle/Triangle/… (не нейросеть).
  ...
  // Шаг 5: рисование bbox и подписей на копии исходника.
  if (m_annotator) {
